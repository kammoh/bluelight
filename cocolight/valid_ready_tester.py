...
from cocotb.triggers import First, Join, ReadOnly, RisingEdge, Timer, FallingEdge
...
class ForkJoinBase:
    ...
    async def fork(self) -> None:
        self._forked = cocotb.start_soon(self.run())
    ...
    async def join(self, timeout=None) -> None:
        if self._forked is not None:
            ...
            self.log.error(f"{self.name} joined without being previously forked!")
        ...
        self.drivers = SimpleNamespace(
            **{
                k: ValidReadyDriver(
                    ...
                    self.clock,
                    debug=debug,
                    min_stalls=min_in_stalls,
                    max_stalls=max_in_stalls,
                )
                for k in input_buses
            }
        )
        self.monitors = SimpleNamespace(
            **{
                bus_name: ValidReadyMonitor(
                    dut=dut,
                    name=bus_name,
                    clock=self.clock,
                    debug=debug,
                    min_stalls=min_out_stalls,
                    max_stalls=max_out_stalls,
                )
                for bus_name in output_buses
            }
        )

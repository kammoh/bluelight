...
static void phoprime(block Y, block M, block X, block C, int no_of_bytes) {
    xor_block(C,Y,M,no_of_bytes);
    pho1(X,Y,C,no_of_bytes);
}
...
static int cofb_crypt(unsigned char *out, unsigned char *k, unsigned char *n,
    ...
    dump_block("before pho1 Y", Y);
    /* X[i] = (pad(A[i]) + G(Y[i-1])) + offset */
    pho1(input,Y,a,alen);
    dump_block("AFTER pho1 X", input);

    dump_halfblock("+offset", offset);

    xor_topbar_block(input, input, offset);
    dump_block("AFTER xor_top X", input);
    /* Y[a] = E(X[a]) */
    giftb128(input, k, Y);
    ...
    if(!emptyM){
        printf("not empty message, last block\n");
        dump_halfblock("offset 0", offset);
        /* full block: offset = 3*offset */
        /* empty data / partial block: offset = 3^2*offset */
        triple_half_block(offset,offset);
        dump_halfblock("offset 1", offset);
        if(inlen%16!=0){
            printf("partial message block (x9)\n");
            triple_half_block(offset,offset);
            dump_halfblock("offset 2", offset);
        }
        /* last block */
        /* C[m] = Y[m+a-1] + M[m]*/
        ...
        printf("after final pho ");
        dump_block("out", out);
        // dump_block(".. Y", Y);

        dump_block("input (before xor)", input);
        dump_halfblock("offset (before xor)", offset);
        xor_topbar_block(input,input,offset);
        dump_block("input (after xor)", input);
        printf("tag perm\n");
        /* T = E(X[m+a]) */
        giftb128(input, k, Y);
    }
    ...
    if (encrypting) {
        memcpy(out + inlen, Y, TAGBYTES);
        return 0;
    } else {
        return (memcmp(in + inlen,Y,TAGBYTES) ? -1 : 0);     /* Check for validity */
    }
}
